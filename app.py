...
df_top_10_scores = df_top_10_scores[['Round', 'Team Name', 'Points', 'Opponent Team Name', 'Opponent Points']]

df_fixture_difficulty = df_fixture_results.groupby('Team Name')['Opponent Rank'].agg('mean').sort_values(ascending=True)
...
df_most_total_injuries = df_injury_list.groupby(['Team Name'])['pts'].agg(['mean', 'count']).sort_values('count', ascending=False)
...
df_all_league_match_data = pd.read_csv("inputs/all_matches_detailed.csv").sort_values(['Round', 'Match Name', 'Team ID', 'On Field?', 'pos', 'pts'], ascending=[True, True, True, False, True, False])

unique_player_data = df_all_league_match_data.loc[(df_all_league_match_data['Round'] <= 21) & (df_all_league_match_data['On Field?'] == True)]
unique_player_count_pivot = unique_player_data.pivot_table(index='Team Name', values=('id'), aggfunc=lambda x: len(x.unique())).sort_values(by=['id'], ascending=False).reset_index()

bench_list = df_all_league_match_data.loc[(df_all_league_match_data['Round'] <= 21) & (~df_all_league_match_data['On Field?'] == True)].replace(0, np.NaN)
...
scoring_bench_player_count['Utilisation'] = round(scoring_bench_player_count['Utilisation'],1)
scoring_bench_player_count = scoring_bench_player_count.sort_values(by=['Utilisation'], ascending=False)

df_match_data_with_fanfooty_info = pd.read_csv(r'inputs/df_match_data_with_fanfooty_info.csv')
bench_list_2 = df_match_data_with_fanfooty_info.loc[(df_match_data_with_fanfooty_info["Round_x"] <= 21) & (~df_match_data_with_fanfooty_info['On Field?'] == True) & (df_match_data_with_fanfooty_info['pts'] > 0) & (df_match_data_with_fanfooty_info['Injured'] == False)]
...
df_highest_bench_diff = bench_list_2.nlargest(10, 'Diff')[['Round_x', 'Match Name', 'Team Name', 'Full Name_x', 'pts', 'Played...', 'Actually got...', 'Diff']]
                  ...
                  [
                      dcc.Graph(
                          figure={
                              "data": [
                                  dict(
                                      x=df_combined_weekly_ladders[df_combined_weekly_ladders['Team Name'] == i]['Round'],
                                      y=df_combined_weekly_ladders[df_combined_weekly_ladders['Team Name'] == i]['rank_number'],
                                      name=i,
                                      line=dict(width=5)
                        ...
                        dcc.Graph(
                            figure={
                                "data": [
                                    go.Box(
                                        dict(
                                            y=df_fixture_results[df_fixture_results["Team Name"] == i]['Points'],
                                            name=i
                                        )
                    ...
                    [
                        ...
                        html.Div(children=[
                            dash_table.DataTable(
                                ...
                                columns=[{"name": i, "id": i} for i in df_highest_bench_diff.columns],
