...
def sc_login_selenium():
    ...
    driver.find_element_by_xpath(
        '//*[@id="mat-dialog-0"]/ng-component/div/div[2]/div/div[3]/button[2]'
    ).click()
    driver.find_element_by_name("email").send_keys(sc_user)
    driver.find_element_by_name("password").send_keys(sc_pass)
    driver.find_element_by_xpath(
        '//*[@id="r-login-container"]/div/div[1]/form/div/div/button'
    ).click()
    return driver
...
def download_all_match_results():
    ...
    for round_number in range(1, rounds + 1):
        for opponent_id in teams:
            ...
            match_url = (
                "https://supercoach.heraldsun.com.au/afl/draft/service/match_centre/update?"
            ...
            match_json = (match_source.split(remove_from_start))[1].split(
                remove_from_end
            )[0]
            match_json = json.loads(match_json)
            file_name = "matches/round {} opponent {}.json".format(
                round_number, opponent_id
            )
            print(file_name)
...
def get_match_name(round_number, team_id):
    ...
    team_one = fixture_df[
        (fixture_df["Round"] == round_number) & (fixture_df["Team One ID"] == team_id)
    ]
    team_two = fixture_df[
        (fixture_df["Round"] == round_number) & (fixture_df["Team Two ID"] == team_id)
    ]
    if len(team_one) > 0:
        ...
        print(
            "No matching round and team ID for round {} and team {}".format(
    ...
    for file in match_file_list:
        with open(file, "r") as f:
            ...
            if current_round != round_number:
                ...
                my_on_field_players = match_json["team"]["player_list"]["field"][
                    "players"
                ]
                my_on_field_df = get_player_info(
                    round_number, team_id, team_name, my_on_field_players, True
                )
                my_off_field_players = match_json["team"]["player_list"]["bench"][
                    "players"
                ]
                my_off_field_df = get_player_info(
                    round_number, team_id, team_name, my_off_field_players, False
                )
            ...
            other_on_field_players = match_json["opponent"]["player_list"]["field"][
                "players"
            ]
            other_on_field_df = get_player_info(
                round_number, team_id, team_name, other_on_field_players, True
            )
            other_off_field_players = match_json["opponent"]["player_list"]["bench"][
                "players"
            ]
            other_off_field_df = get_player_info(
                round_number, team_id, team_name, other_off_field_players, False
            )
